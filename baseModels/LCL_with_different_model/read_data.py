...
class Goemotions(Dataset):
    def __init__(self, dataset, num_labels, args, sent_len=128):
        ...
        if args.weighting_model:
            self.weighting_tokenizer = AutoTokenizer.from_pretrained(args.weighting_model)
    ...
    def collate_fn(self, batch):
        ...
        weighting_tokenized_tokens = self.weighting_tokenizer(texts, padding=True, truncation=True,
                                                              max_length=self.sent_len)

        tokens = torch.tensor(
            tokenized_tokens["input_ids"])
        weighting_tokens = torch.tensor(
            weighting_tokenized_tokens["input_ids"])
        attn_mask = torch.tensor(tokenized_tokens["attention_mask"])
        weighting_attn_mask = torch.tensor(weighting_tokenized_tokens["attention_mask"])

        del batch
        return tokens, attn_mask, weighting_tokens, weighting_attn_mask, labels
...
if __name__ == '__main__':
    ...
    for filename in ["../../data/train.tsv", '../../data/goemotions_train.csv']:
        ...
        for batch_idx, (
        batch_input, batch_attn_mask, batch_weighting_input, batch_weighting_attn_mask, batch_label) in enumerate(
                train_dataloader):
            print(batch_idx)
            print(batch_input)
            print(batch_weighting_input)
            print(batch_label)
            print(batch_attn_mask)
            print(batch_weighting_attn_mask)
            break
