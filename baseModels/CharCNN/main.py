...
def get_args():
    ...
    # Optimizer
    parser.add_argument(
        "-p", "--optimizer", type=str, choices=["sgd", "adam"], default="sgd"
    )
    parser.add_argument("-b", "--batch_size", type=int, default=128)
    parser.add_argument("-n", "--num_epochs", type=int, default=20)
    parser.add_argument(
        "-l", "--lr", type=float, default=0.001
    )  # recommended learning rate for sgd is 0.01, while for adam is 0.001
    parser.add_argument(
        "-y",
        "--es_min_delta",
        type=float,
    ...
    parser.add_argument(
        "-o", "--output", type=str, default="output", help="path to output folder"
    )
...
def main(options):
    ...
    test_params = {"batch_size": options.batch_size, "shuffle": False, "num_workers": 0}
    """
    Load the training set and validation set
    """
    training_set = MyDataset(
        options.input + "train" + "_emotion_group.tsv", options.max_length
    )
    test_set = MyDataset(
        options.input + "dev" + "_emotion_group.tsv", options.max_length
    )
    ...
    for epoch in range(options.num_epochs):
        ...
        test_metrics = get_evaluation(
            te_label, te_pred.numpy(), list_metrics=["accuracy", "confusion_matrix"]
        )
        print(
            "Epoch: {}/{}, Lr: {}, Loss: {}, Accuracy: {}".format(
                epoch + 1,
                options.num_epochs,
                optimizer.param_groups[0]["lr"],
                te_loss,
        ...
        if te_loss + options.es_min_delta < best_loss:
            ...
            torch.save(
                model,
                "{}/char-cnn_{}_{}".format(
                    options.output, options.dataset, options.feature
                ),
            )
        # Early stopping
