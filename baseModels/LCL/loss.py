...
### Credit from https://github.com/varsha33/LCL_loss
...
# Out multi-ouput implementation
class LCL(nn.Module):
    ...
    def forward(self, features, labels=None, weights=None,mask=None):
        """
        Inputs:
            features: [batch_size, hidden_states]
            labels: [[1,0,0,0],[0,1,0,1]]  [batch_size, num_labels]
            weights: weighting matrix. [batch_size, batch_size]
        ...
        weights = F.softmax(weights,dim=1)
        features, weights = torch.cat((features, features),0), torch.cat((weights, weights),0)  # Augment the batch size
        batch_size = features.shape[0]  # Augmented batch size
        ...
        if labels is not None and mask is not None:
            ...
            labels = torch.cat((labels, labels), 0)  # Augment the batch size
            mask = torch.matmul(labels, labels.T)  # compute mask matrix
            mask = torch.where(mask==0, 0, 1)  # Avoid adding more weight on samples
        else:
        ...
        weighted_mask = torch.matmul(weights, labels.T)

        weighted_mask = weighted_mask * logits_mask
        pos_weighted_mask = weighted_mask * mask  # Compute the weighted matrix
